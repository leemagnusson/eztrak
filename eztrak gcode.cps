...
minimumCircularRadius = spatial(0.0025, MM);
maximumCircularRadius = spatial(1000, MM);
minimumCircularSweep = toRad(0.0025);
...
var xyzFormat = createFormat({decimals:(unit == MM ? 4 : 6), forceDecimal:true});
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (properties.useToolChanger) {
      writeBlock("T" + toolFormat.format(tool.number), mFormat.format(26)); // 26 for tool change at clearance point
